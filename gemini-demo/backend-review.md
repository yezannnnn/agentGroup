...
| 🟡    | 必须使用 SLF4J 接口（`org.slf4j.Logger`），禁止直接用 `System.out.println` 或 Log4j 实现类 | `System.out.println(...)`                   | `private static final Logger log = LoggerFactory.getLogger(XxxServiceImpl.class);` 或 Lombok `@Slf4j` |
